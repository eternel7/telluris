...
- **A/B** : jusqu'à X chemins (`voiesChemins`, champ `#voies-nb-chemins` 1-8, 3 par défaut), chacun forcé par un **passage différent** — cases du chemin précédent ET leurs voisines retirées du graphe, hors abords de A/B (`VOIES_ABORDS`) ; ⚠️ arrêt dès qu'un chemin n'ajoute aucune case (sinon boucle infinie sur A/B voisins) + **coupe minimale en cases** (`voiesCoupeMin`, flot max sur graphe dédoublé) — la brèche de 2-3 cases qu'aucun goulot ne montre. ⚠️ Visée interceptée en **tête** de `mousedown`/`touchstart` (ne peint rien), refusée si le tracé est périmé ou qu'une visée Lieux est armée ; A/B survivent à ↻ Recalculer.
