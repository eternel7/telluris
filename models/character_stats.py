...
# Contrat d'UNE MISSION (signé à la guilde) : troisième mode de lien, entre le contrat
# ordinaire (sans terme, part 10-30 %) et l'engagement durable (part nulle, hors plafond).
# Il se PAIE d'avance en cuivre — COUT + COUT_PAR_NIVEAU × niveau de la recrue — contre une
# part de butin RÉDUITE (part brute × PART_FACTEUR, plancher PART_MIN, l'affinité restant
# déductible par-dessus). Il s'achève de lui-même à la première quête rendue DANS une guilde.
# ⚠️ PART_FACTEUR est clampé [0.0, 1.0] : au-dessus de 1, un contrat court coûterait à la
# fois de l'argent ET plus de butin qu'un contrat ordinaire — l'inverse de son objet.
RECRUTEMENT_MISSION_COUT_CUIVRE: int = 300
RECRUTEMENT_MISSION_COUT_PAR_NIVEAU: int = 200
RECRUTEMENT_MISSION_PART_FACTEUR: float = 0.5
RECRUTEMENT_MISSION_PART_MIN: int = 0
...
# Déversement DÉTERMINISTE du surplus au pool de la cité : ce qui dépasse le stock cible d'un
# rayon, et que le lieu ne consomme pas lui-même, part à la ville à chaque tick — sans tirage
# ni arrondi. C'est ce qui fait circuler les sous-produits du dépeçage (os, cuir, tendons) du
# boucher vers le tabletier. 0 = comportement d'avant (seuls les achats PNJ alimentent le pool).
FLUX_SURPLUS_PART: float = 1.0
# Part d'une ligne du pool qu'UN atelier emporte à son tick. À 1 le premier arrivé rafle tout
...
def current_world_variables() -> dict:
	...
	return {
		...
		"RECRUTEMENT_MISSION_COUT_CUIVRE": RECRUTEMENT_MISSION_COUT_CUIVRE,
		"RECRUTEMENT_MISSION_COUT_PAR_NIVEAU": RECRUTEMENT_MISSION_COUT_PAR_NIVEAU,
		"RECRUTEMENT_MISSION_PART_FACTEUR": RECRUTEMENT_MISSION_PART_FACTEUR,
		"RECRUTEMENT_MISSION_PART_MIN": RECRUTEMENT_MISSION_PART_MIN,
...
def load_world_variables() -> dict:
	...
	global RECRUTEMENT_MISSION_COUT_CUIVRE, RECRUTEMENT_MISSION_COUT_PAR_NIVEAU
	global RECRUTEMENT_MISSION_PART_FACTEUR, RECRUTEMENT_MISSION_PART_MIN
	...
	RECRUTEMENT_MISSION_COUT_CUIVRE = max(0, int(v.get("RECRUTEMENT_MISSION_COUT_CUIVRE", RECRUTEMENT_MISSION_COUT_CUIVRE)))
	RECRUTEMENT_MISSION_COUT_PAR_NIVEAU = max(0, int(v.get("RECRUTEMENT_MISSION_COUT_PAR_NIVEAU", RECRUTEMENT_MISSION_COUT_PAR_NIVEAU)))
	# Clampé [0, 1] : > 1 rendrait le contrat court plus cher EN BUTIN que l'ordinaire.
	RECRUTEMENT_MISSION_PART_FACTEUR = max(0.0, min(1.0, float(v.get("RECRUTEMENT_MISSION_PART_FACTEUR", RECRUTEMENT_MISSION_PART_FACTEUR))))
	RECRUTEMENT_MISSION_PART_MIN = max(0, int(v.get("RECRUTEMENT_MISSION_PART_MIN", RECRUTEMENT_MISSION_PART_MIN)))
