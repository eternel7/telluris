...
par le moteur réel (`utils/competences.normaliser_competence`) et contrôle les invariants
listés plus bas. Il échoue en code 1.

---

## Pourquoi

La base ne contient que **38 compétences, toutes de niveau 0 ou 1**.
...
`_bonus_dict` (`utils/sorts.py:45-82`) est une **liste blanche fermée**. Une clé inventée
(`crit`, `actions`, `resistance_feu`…) produit un doc **silencieusement inerte**. Vocabulaire
disponible, et lui seul :

| dans `effets` | dans le doc |
|---|---|
| `degats` `"2D8+4"` · `pv` · `pm` · `regen_pv` · `regen_pm` | `mode` (passive/active) · `cout_pm` |
| `buffs` `{V,F,R,Ag,Vol,Int,Cha,Ch: ±int}` · `duree` | `cible` (soi/ennemi/allie) · `jet` (cc/cd/magique) |
| `esquive` · `furtivite` | `portee` · `condition.battle_map_tags` · `animation` |

### Les cinq pièges qui ont cadré la rédaction

1. **Une passive `condition`nnée est EXCLUE du repli permanent** (`competences.py:163`) : seule
   sa `furtivite` est lue, au snapshot de combat (`furtivite_passive:204-214`). ⇒ **une passive à
   `condition` ne porte QUE `furtivite`**. Une seule compétence de ce document en use
   (`competence:eclaireur`), sur le modèle exact de `competence:furtivite_sylvestre`.
2. **`furtivite` est ignorée hors combat**, et se prend en **MAX**, jamais en somme.
3. **`Cha` n'a aucune dérivée de combat** (`compute_derived_stats`) : un buff de Cha est social
   et marchand. Réservé aux vocations dont c'est l'identité — ménestrel, duelliste, druide
   (emprise), prêtre, paladin.
...
5. **Deux actives ne cumulent pas sur la même caract** (meilleur bonus + pire malus retenus,
   `consommables.cumul_effets`). Passives et équipement, eux, s'additionnent.
...
de chaque entrée dit ce qui a été retenu à la place. Un `+1 en Ag au-dessus du maximum` devient
un gros buff d'Ag (les buffs ne passent pas par `compute_stat_cap` — ils s'ajoutent après, cf.
convention 3 : un buff n'ouvre jamais un plafond, mais il produit bien l'effet en jeu).

### Deux leviers supportés et jamais employés par les compétences actuelles

- **le debuff pur sur `cible:"ennemi"`** — `{buffs:{F:-15}, duree:3}` sans dégâts : accepté par
  `resolve_action` (`combat.py:2838`, `dmg = 0`, « la prise porte »). Les sorts s'en servent
  déjà, les compétences jamais. Onze entrées de ce document en vivent ;
...
(`_degats_competence:1610`, `_portee_competence:1631`) — `cd` et `magique` en sont exclus. Le
bloc `degats` d'une frappe de contact se lit donc **au-dessus** du `degats_cc` du porteur : à PM
égal, une entrée `cc` est plus forte qu'une entrée `magique` de même notation.
...
| **Active — PM** | **15** | **25** | **40** |
| offensive | `2D8+4` (≈ 13) | `3D8+8` (≈ 21) | `5D10+18…20` (≈ 45-48) |
...
⚠️ Un gros buff de `R` ou de `Vol` **re-clampe PV/PM à son expiration** (`combat.py:148-151`,
plancher `pv_max ≥ 1`) : c'est voulu et déjà éprouvé par les sorts, mais cela se voit sur les
capstones de chaman et de démoniste, qui montent `R` de +15 à +20.
...
1. Chaque bloc passe `normaliser_competence` **sans perte de clé** — ni dans `effets`, ni au
   premier niveau du doc, ni dans `buffs`. Une clé inventée (`crit_bonus`, un `magie` recopié
   d'un sort, un buff sur une caract qui n'existe pas) ne lève rien et **disparaît en silence** :
   c'est le piège n°1 du contenu, et la raison d'être du vérificateur.
2. Aucun `_id` en double, aucune collision avec les 38 `competence:*` du dump.
3. Exactement **6 entrées par vocation** : une passive et une active par niveau 3, 6, 10.
4. `vocation` ∈ les 20 ids de `rules:vocations` ; `niveau` ∈ {3, 6, 10}.
5. Toute passive portant `condition` n'a **que** `furtivite` dans ses effets.
6. Toute active `cible:"ennemi"` porte `degats` **ou** une part durative — sinon
   `resolve_action` la refuse.
7. Toute active passe `competence_utilisable_combat` **ou**
   `competence_utilisable_exploration` (une active qui échoue aux deux est inutilisable partout).
8. Aucune passive ne porte `cout_pm`, `cible`, `jet` ou `portee` — champs sans effet sur une
   passive, et trompeurs à la relecture.
...
**Garde de fer** 🛡️ · active · 25 PM · `soi` / portée 1
```json
{"_id": "competence:garde_de_fer", "type": "competence", "nom": "Garde de fer", "icon": "🛡️",
 ...
 "cible": "soi", "portee": 1,
 "effets": {"buffs": {"R": 18}, "esquive": 8, "duree": 5}}
```
...
**Brise-ligne** 💥 · active · 40 PM · `ennemi` / `cc` / portée 1
```json
...
**Spasme de furie** 🪓 · active · 40 PM · `ennemi` / `cc` / portée 1
```json
...
**Parade de maître** ⚔️ · active · 25 PM · `soi` / portée 1
```json
{"_id": "competence:parade_de_maitre", "type": "competence", "nom": "Parade de maître", "icon": "⚔️",
 ...
 "cible": "soi", "portee": 1,
 "effets": {"buffs": {"Ag": 10}, "esquive": 16, "duree": 5}}
```
...
**Souffle du Sensei** 🌬️ · active · 40 PM · `allie` / portée 2
```json
{"_id": "competence:souffle_du_sensei", "type": "competence", "nom": "Souffle du Sensei", "icon": "🌬️",
 ...
 "effets": {"pv": 45, "pm": 20, "regen_pv": 5, "buffs": {"Vol": 20}, "duree": 5}}
```
...
**Serment du martyr** 🩸 · active · 25 PM · `allie` / portée 2
```json
{"_id": "competence:serment_du_martyr", "type": "competence", "nom": "Serment du martyr", "icon": "🩸",
 ...
 "cible": "allie", "portee": 2,
 "effets": {"pv": 30, "buffs": {"R": 12}, "regen_pv": 4, "duree": 5}}
```
...
**Lumière du Purificateur** ☀️ · active · 40 PM · `ennemi` / `magique` / portée 6
```json
{"_id": "competence:lumiere_du_purificateur", "type": "competence", "nom": "Lumière du Purificateur", "icon": "☀️",
 ...
 "effets": {"degats": "4D10+12", "buffs": {"Vol": -20}, "duree": 3}}
```
...
**Verdict** ⚖️ · active · 40 PM · `ennemi` / `cc` / portée 1
```json
{"_id": "competence:verdict", "type": "competence", "nom": "Verdict", "icon": "⚖️",
 ...
 "cible": "ennemi", "jet": "cc", "portee": 1,
 "effets": {"degats": "4D10+16", "buffs": {"R": -15}, "duree": 3}}
```
...
**Tueur de démon** 😈 · active · 40 PM · `ennemi` / `cc` / portée 1
```json
{"_id": "competence:tueur_de_demon", "type": "competence", "nom": "Tueur de démon", "icon": "😈",
 ...
 "cible": "ennemi", "jet": "cc", "portee": 1,
 "effets": {"degats": "5D10+18", "buffs": {"Vol": -20}, "duree": 3}}
```
...
**Hymne de l'Étoile** 🌟 · active · 25 PM · `allie` / portée 5
```json
{"_id": "competence:hymne_de_l_etoile", "type": "competence", "nom": "Hymne de l'Étoile", "icon": "🌟",
 ...
 "cible": "allie", "portee": 5,
 "effets": {"pv": 20, "buffs": {"Vol": 18, "Cha": 10}, "regen_pv": 3, "duree": 5}}
```
*Prolonge `competence:refrain_de_ralliement` (niv. 1) — même portée longue, même cible alliée.*
...
**Chant du Prodige** 🎼 · active · 40 PM · `ennemi` / `magique` / portée 6
```json
{"_id": "competence:chant_du_prodige", "type": "competence", "nom": "Chant du Prodige", "icon": "🎼",
 ...
 "cible": "ennemi", "jet": "magique", "portee": 6,
 "effets": {"degats": "2D8", "buffs": {"F": -20, "Ag": -20, "Vol": -15}, "duree": 4}}
```
*Source : « Prodige » (niv. 5), « charme de faiblesse ». Le capstone du ménestrel est le plus lourd debuff du jeu, pas un burst — c'est son identité.*
...
**Oracle** 👁️‍🗨️ · active · 40 PM · `allie` / portée 6
```json
{"_id": "competence:oracle", "type": "competence", "nom": "Oracle", "icon": "👁️‍🗨️",
 ...
 "effets": {"pv": 45, "pm": 25, "buffs": {"Ch": 20, "Vol": 15}, "regen_pv": 5, "regen_pm": 3, "duree": 5}}
```
...
**Sève vive** 🌿 · active · 25 PM · `allie` / portée 3
```json
...
**Homme-tempête** ⛈️ · active · 40 PM · `ennemi` / `magique` / portée 10
```json
{"_id": "competence:homme_tempete", "type": "competence", "nom": "Homme-tempête", "icon": "⛈️",
 ...
 "effets": {"degats": "5D10+16", "buffs": {"V": -3}, "duree": 3}}
```
...
**Décharge primordiale** ⚡ · active · 25 PM · `ennemi` / `magique` / portée 10
```json
...
**Courroux des éléments** 🌋 · active · 40 PM · `ennemi` / `magique` / portée 12
```json
{"_id": "competence:courroux_des_elements", "type": "competence", "nom": "Courroux des éléments", "icon": "🌋",
 ...
 "effets": {"degats": "5D12+20", "buffs": {"R": -15}, "duree": 3}}
```
...
**Rupture arcanique** 💥 · active · 40 PM · `ennemi` / `magique` / portée 10
```json
{"_id": "competence:rupture_arcanique", "type": "competence", "nom": "Rupture arcanique", "icon": "💥",
 ...
 "cible": "ennemi", "jet": "magique", "portee": 10,
 "effets": {"degats": "5D10+20", "buffs": {"Vol": -15}, "duree": 3}}
```
...
**Théâtre des ombres** 🎭 · active · 40 PM · `ennemi` / `magique` / portée 10
```json
{"_id": "competence:theatre_des_ombres", "type": "competence", "nom": "Théâtre des ombres", "icon": "🎭",
 ...
 "cible": "ennemi", "jet": "magique", "portee": 10,
 "effets": {"degats": "3D10", "buffs": {"F": -20, "Ag": -20, "Int": -15}, "duree": 4}}
```
...
**Étreinte du tombeau** ⚱️ · active · 40 PM · `ennemi` / `magique` / portée 8
```json
{"_id": "competence:etreinte_du_tombeau", "type": "competence", "nom": "Étreinte du tombeau", "icon": "⚱️",
 ...
 "cible": "ennemi", "jet": "magique", "portee": 8,
 "effets": {"degats": "5D10+18", "buffs": {"R": -20, "V": -3}, "duree": 4}}
```
...
**Griffe du familier** 👹 · active · 25 PM · `ennemi` / `magique` / portée 6
```json
...
**Œuvre de l'Enchanteur** 💎 · active · 40 PM · `soi` / portée 1
```json
{"_id": "competence:oeuvre_de_l_enchanteur", "type": "competence", "nom": "Œuvre de l'Enchanteur", "icon": "💎",
 ...
 "cible": "soi", "portee": 1,
 "effets": {"pm": 30, "buffs": {"Int": 22, "Vol": 15}, "regen_pm": 5, "duree": 5}}
```
...
| burst pur | — | guerrier, barbare, duelliste, assassin |
| burst + entrave | — | voleur, templier, répurgateur, paladin, druide, élémentaliste, mage, illusionniste, nécromancien, forestier |
| debuff massif | — | ménestrel |
| soutien d'allié | — | moine, prêtre |
| métamorphose / possession de soi | — | chaman, démoniste, lettré |
