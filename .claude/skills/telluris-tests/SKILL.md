...
| `test_saut_client` | ciblage de SAUT (`casesSaut`/`peindreCasesSaut` du combat) | le seul ciblage qui vise une CASE : cases offertes qui divergent de `_verifier_saut` (le mur doit se franchir, pas s'habiter) ; `pointer-events`/`z-index` oubliés sur `.zone-case.destination`, donc un clic jamais reçu ; cases laissées peintes après annulation |
