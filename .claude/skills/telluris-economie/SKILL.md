...
Entrée d'inventaire = **string legacy** `"item:xxx"` (poids = min) **ou objet** `{"item": "item:xxx", "poids": <nb>}` ; le champ `poids` d'un doc `item:*` est un nombre OU `[min,max]`. Helpers `item_ref_id`/`poids_bounds`/`item_ref_weight`/`resolve_item_ref` couverts par `tests/test_item_ref.py`. Tout exemplaire CRÉÉ (butin, don PNJ, affaires d'une escorte, course écrite, achat au marché — réf `{item, poids}` seulement pour un doc à fourchette —, commande) tire son poids UNE fois avec `characters.tirer_poids(item, rand_fn)` ; seule `item_ref_weight` (réf nue, relue à chaque lecture) garde le min. Le catalogue de commande envoie `poids` (min, nombre) + `poids_max` pour une fourchette (`commande.poids_catalogue`). Une référence peut porter **`lieu_parent`** = le lieu qui a délivré CET exemplaire (`item_ref_lieu`), dont `resolve_item_ref` dérive un label localisé (« Carte d'aventurier (Auxerre) ») sans jamais toucher le doc item générique.
...
**Livraison par DÉCLARATION — `APPRO_EXTRA`** `{categorie: [clés]}` (variable de monde, fusion comprise) : une matière qu'aucune recette de la catégorie ne consomme y est quand même livrée (réserve + comptoir) et rachetée au joueur (`appro_extra_categorie`, unie à `besoins_categorie` et aux deux `appro_leaves_*`). Seul usage au 24/09 : les lingots de mithril/orichalque/adamantite au `grand_arsenal`, matières de sur-mesure sans recette consommatrice. ⚠️ Leur débit se règle **sous l'id** (`APPRO_DEBIT["item:mithril"]`) : sinon repli sur la sous-catégorie `metaux_precieux`, puis sur le défaut 5. Poussé en base par `dev/gen_depecage_tags.py` (seul porteur du doc `rules:world_variables` entier). Verrouillé par `tests/test_appro_extra.py`.
...
- ⚠️ **Une matière doit pouvoir S'OBTENIR** : le sur-mesure propose tout le catalogue du monde, donc une matière sans source donne une commande à jamais `en_attente_materiaux`. `dev/audit_economy.py` §6 (hypothèse J) les liste — à relancer après tout ajout de matière. Sources ajoutées le 24/09 : `jsons/recettes_matieres_sans_source_a_importer.json` (poison, fiole de sang de bête, poudre d'améthyste, encre magique, fragments d'âme) + `item:Poison_de_base` sur le tag de dépeçage `venin`.
