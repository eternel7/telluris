...
- **Épreuve d'APPORT** (`services.rang.apport = {rang_vise, items, id?, titre?, recompenses}`) : rang gagné en REMETTANT un objet de la liste, sans chasse. Ouverte au seul cran du dessous (unique sans champ neuf), elle **franchit le plafond** du comptoir, archivée sous `apport_quete_id`. Flags `rang_apport_offert`/`rang_apport_possible`, action `rang/apporter` → nœud `apporte`, placeholder `{objet}`. Objet cherché chez le principal **puis ses montures** (portions de Tarasque de 70-520 kg) ; ⚠️ **jamais un compagnon** : XP partagée et fin de contrat rechargent le groupe, deux dicts du même doc. Contenu : `jsons/pnj_eleonore_de_vaugirard_a_importer.json` (Grand Relais, Lutèce).
...
- Présences **CUMULABLES** : chaque entrée tire SA probabilité, indépendamment des autres ; tous ceux qui passent sont là ensemble, une ligne 🗣 chacun dans les actions (ordre de la liste du lieu, un id retenu une seule fois). `pnj_present = {lieu, characters:[…]}` — forme d'avant (`character: id|None`) relue telle quelle. Le client dit à QUI il parle (`pnj_id`, query du GET / corps du POST) ; à défaut, le premier présent. ⚠️ Les offres transitoires du lieu restent UNIQUES : le donneur se **cherche** parmi les présents (`transport.donneur_present`, `escorte.donneur_present` — l'ordre des trois branches y fait foi), sinon un PNJ muet en tête de liste ferait taire l'offre écrite de son voisin.
- Doc `type:"pnj"` : arbre de dialogue (`noeud_depart`, `noeuds`, choix filtrés serveur par condition) + **services** `direction` (chemin d'un lieu de la ville, cf. Saisie libre), `soin` (gratuit/efficace au-dessus d'un seuil de relation), `don` (contrôle de charge ; `lieu_parent:"auto"` + `rang_guilde` + `unique` = inscription sans épreuve, flag `don_recu` = l'exemplaire est dans le sac du principal), `acces`, `rang`, `commission`, `transport`, `escorte`.
...
- **Saisie libre** : un choix `"saisie": true` s'affiche en champ texte + bouton (`renderPnjNoeud`, `choisirPnjSaisie`) ; sa valeur part en `saisie` dans le POST. Seul lecteur : le service **`direction`** (`noeuds:{trouve, proche, inconnu}`) — le PNJ cherche le nom parmi les lieux du même `lieu_parent` que sa boutique (`transport.chercher_lieu_nomme`, `difflib`, ≤ `SUGGESTIONS_MAX`) et répond avec la phrase des courses (`indice_destination` + `texte_indice` → `{direction}`) ; `{recherche}`/`{suggestions}` sinon. ⚠️ La saisie ne va jamais dans un `label` (innerHTML) : seulement dans `texte` (`textContent`). Borne `transport.RECHERCHE_LIEU_MAX`, recopiée par le `maxlength` du client. Contenu : `dev/gen_direction_marchands.py` (les 48 `pnj:marchand_*`).
...
- **Pas de condition de sortie** : `_check_victory` ignoré, `fuir` refusé. Action `emprunter` (gratuite) : le **principal** SUR la case, les autres combattants debout en **file continue** derrière lui — chacun adjacent à un autre, de proche en proche (un groupe de 7 s'étire sur 7 cases, diagonale comprise ; principal à terre → la file part de tout combattant debout sur la case) (`passage_franchissable`, exposé en `franchissable` par `annoter_passages`). ⚠️ Jamais « tous à ≤ 1 du passage » : une sortie au fond d'un couloir d'une case n'a qu'une voisine, un groupe de 3 y restait enfermé (verrouillé par `test_un_couloir_en_cul_de_sac_se_franchit_en_file`). Vers un étage : `routers/combat._passer_a_l_etage` → `combat.changer_d_etage` (joueurs/PV/PM/effets conservés, monstres archivés et renumérotés, carcasses laissées en bas, client recharge la page). Vers la surface : `victoire` + `sortie` appliquée par `finalize_combat` (XP, kills, bestiaire sur tous les étages). Défaite : retour au lieu d'origine, sans XP.
- **Butin d'un étage vidé** : `emprunter` vers un AUTRE étage sans ennemi vivant → sans `attributions`, renvoie `{butin_etage, passage_id}` sans bouger (poids tirés une fois, mémorisés sur `etages.butin_etage`) ; le client ouvre l'overlay de répartition (✕/Échap = rester) et rappelle `emprunter` avec `attributions` (liste vide = tout laisser). Les carcasses vont au `butin_ramasse` du SNAPSHOT (versé au sac par `_finalize_membre` / `_finalize_monture`, toute issue), refus global en surcharge ; le reste demeure à l'étage. La sortie vers la surface garde `butin_disponible` (morts de l'étage courant seulement).
...
- UI (`renderPassages`, combat_telluris) : un bouton par passage **empruntable maintenant** seulement (à son tour, `franchissable`), dans le **bandeau d'initiative** (`.tour-header`, entre `#initiative-strip` et le 🔊) et non plus dans la barre d'action, où la rangée coûtait une ligne vide. `margin-left: auto` : un bouton qui paraît prend la marge libre sans décaler les badges, et la hauteur du bandeau reste celle des badges (§17) — plus de cale invisible. Bouton = pastille dorée `.passage-btn` (teinte de la case de passage), **icône seule sous 768 px** (nom en `title`/`aria-label`). ⚠️ C'est le passage qui cède la place (`flex: 0 4 auto`), jamais `#initiative-strip` — et pas de `min-width: 0` sur la rangée, qui rognerait la pastille.
