...
- Doc `type:"pnj"` : arbre de dialogue (`noeud_depart`, `noeuds`, choix filtrés serveur par condition) + **services** `soin` (gratuit/efficace au-dessus d'un seuil de relation), `don` (contrôle de charge ; `lieu_parent:"auto"` + `rang_guilde` + `unique` = inscription sans épreuve, flag `don_recu` = l'exemplaire est dans le sac du principal), `acces`, `rang`, `commission`, `transport`, `escorte`.
